...
func (ctrl *KubeController) getEndpointByName(indexKey string, clientIP net.IP, geoDataFilePath string, geoDataFieldPath ...string) (lep LocalDNSEndpoint) {
	log.Infof("Index key %+v", indexKey)
	// TODO: Handle indexing
	host := strings.Split(indexKey, ":")[0]
	endpoints := ctrl.getEndpointsByCaseInsensitiveName(host, clientIP, geoDataFilePath, geoDataFieldPath...)
	lep = ctrl.margeLocalDNSEndpoints(indexKey, endpoints)
	return lep
}
	...
	for _, obj := range epList {
		...
		for _, extracted := range extracts {
			if strings.EqualFold(extracted.DNSName, host) {
				key := fmt.Sprintf("%s:%s", extracted.DNSName, extracted.RecordType)
...
func (ctrl *KubeController) margeLocalDNSEndpoints(host string, endpoints map[string]LocalDNSEndpoint) LocalDNSEndpoint {
	log.Debugf("ARRA: margeLocalDNSEndpoints: endpoints: %v", endpoints)
	result := LocalDNSEndpoint{
		DNSName: host,
	}
	...
	log.Debugf("ARRA: margeLocalDNSEndpoints: ep: %v", endpoints[host].Targets)
	result.Targets = append(result.Targets, endpoints[host].Targets...)
	return result
}
