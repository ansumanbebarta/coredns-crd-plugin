...
func (gw *Gateway) ServeDNS(_ context.Context, w dns.ResponseWriter, r *dns.Msg) (int, error) {
	...
	log.Infof("Incoming query %s %s", state.QName(), state.Type())
	...
	// Include RecordType as well 
	indexKey = fmt.Sprintf("%s:%s", indexKey, state.Type())
...
// A generates dns.RR for AAAA record
